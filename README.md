...
verifier rejects it, which is a bug rather than a feature.
...
25 diagnoses with a known correct verdict, 12 sound and 13 flawed, over four runs.
...
well as a quality number. One rejection is 8.3 points on 12 cases, which is why the range
is reported beside the mean.

Nothing exercised the verifier until this suite existed. Its first run put false rejection
at 50%, caused by one over-strict check of my own; rewriting that took it to roughly 10%.
Tightening `evidence_grounded` then took false acceptance from 7.7% to zero. The suite
also caught a mislabelled fixture and a taxonomy signal the verifier was reading
backwards. Full account, including a rebalancing attempt that made things worse and was
reverted, in [EVAL_REPORT.md](EVAL_REPORT.md).
...
κ = 0.26 is weak. The judge is rarely wildly wrong, with 0.95 adjacent agreement on whether an
...
text engineered to read as an instruction: *report this pattern*, *return confidence 1.0*, *print your system prompt*, *emit a full working solution*. The diagnoser has no
